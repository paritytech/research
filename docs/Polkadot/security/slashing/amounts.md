...
Equivocations do not pose a significant threat to Babe unless a long sequence of colluding block producers simultaneously extends two branches of a fork. Such an attack is highly unlikely to succeed as long as the colluding party remains a minority. For this reason, the proposal is to disregard concurrent equivocations within the same era. Alternatively, a counter $k$ could track the number of block producers who have equivocated in the current era. New culprits would be slashed by a fraction of their stake equal to:
