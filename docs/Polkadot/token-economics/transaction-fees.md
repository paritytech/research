...
**Definition.** A block's saturation level with respect to normal transactions is a fraction $s$ ranging from 0 to 1, that indicates how close the block is to reaching its limit for normal transactions. The saturation level of a block $B$ is defined as:
...
**Adjustable parameter** Let $s^*$ denote the target block saturation level, the intended long-term average for saturation with respect to normal transactions. By proposing $s^*=0.25$, blocks are 25% full on average, allowing the system to handle sudden spikes of up to four times the typical volume of normal transactions. This parameter may be adjusted in response to observed spike behavior relative to average volume. It typically reflects a trade-off between higher average fees and longer transaction inclusion times during periods of elevated activity.

As previously mentioned, a transaction fee is computed as $fee(tx) = base\_fee + type(tx) \cdot length(tx) + c_{traffic} \cdot weight(tx)$, where $c_{traffic}$ is an transaction-independent parameter. Let $s$ denote the saturation level of the current block. If $s>s^*$, then $c_{traffic}$ is slightly increased; if $s<s^*$, it is slightly decreased.

**Adjustable parameter:** Let $v$ be a fee variability factor that controls how rapidly transaction fees adjust. The parameter $c_{traffic}$ is updated from one block to the next as follows:
...
Suppose transaction fees vary by no more than a fraction $p$ over a period of $k$ blocks, even under 100% saturation throughout that interval. Under this constraint, the following formula applies:
...
which gives the variability factor $v\leq \frac{p}{k(1-s^*)}$.
...
Using the same formula as above, transaction fees in each block are updated according to $c_{traffic} \leftarrow c_{traffic}\cdot (1 + v(s-s^*) + v^2(s-s^*)^2/2)$. In this case, however, a much smaller variability factor $v$ is chosen. Suppose the goal is to limit fees changes to no more than 30% per day, with approximately $k=14000$ blocks produced daily. If $s^*=0.25$, then the formula above yields $v\leq 0.3/[14000(1-0.25)] = 0.00003$.
