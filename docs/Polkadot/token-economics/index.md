...
[^1]: DOT is Polkadot's native token and its main functions are: 1) Economics: Polkadot mints or burns DOT to reward the nodes that run the consensus protocol, fund the treasury, control the inflation rate, and support other economic mechanisms. 2) Slashing: DOT also plays a role in slashing protocols designed to desincentivize attacks or adversarial behaviors. 3) Governance: DOT functions as voting power, allowing DOT holders to express their opinion in governance decisions via referenda. 4) Parachain allocation: Finally, DOT is used to decide which projects receive parachain slots, through auctions and deposits. This section focuses on the first function, while the remaining three are explored in subsequent sections.
